# skills

我日常使用的 agent skills,可在任意支持的 coding agent 中一键安装。

## 安装

通过 [`npx skills`](https://github.com/vercel-labs/skills) 安装:

```bash
# 安装本仓库全部 skill
npx skills add idontdebug/skills

# 只安装某个 skill
npx skills add idontdebug/skills --skill explain-code

# 查看可用的 skill
npx skills add idontdebug/skills --list
```

支持 Claude Code、Codex、Cursor、Droid 等 70+ agent,会自动检测已安装的 agent 并写入对应的 `skills/` 目录。

## Skills

### clarify

需求澄清与方案设计。当你抛出一个模糊的一句话需求("加个通知功能"、"优化一下性能")时,它不会直接动手写代码,而是先摸底代码库现状并判断需求类型/规模,然后以**选择题**的方式一轮轮提问——每个问题给 2-3 个落到真实文件/模块上的候选方案(带 ⭐ 推荐和理由),全程展示决策记录、支持"你定"秒过和"别问了"打包确认,最终产出一份含数据模型、接口设计、逐文件改动清单和验收标准的详细设计文档。小需求(S 规模)不走流程,确认一句直接动手。

每条决策会标注来源(**用户确认 / 默认假设**),出文档前先按异常流/并发/幂等/数据/权限/量级/兼容做一轮遗漏自查,文档写完后自动调用 `spec-review` 对抗评审一轮再交付。

### spec-review

需求/设计文档对抗评审。拿一份需求文档、设计方案或 PRD 说"评审一下"、"还有什么漏洞"即可触发,也是 `clarify` 产出设计方案后的自动收尾环节。扮演挑刺的评审员:先用代码库核实文档里的论断(点名的文件是否存在、"现有系统已支持 X"是否属实),再按内部矛盾/歧义/异常流/并发幂等/数据/权限/兼容/可验证性/现状冲突/默认假设十个维度攻击,输出带严重程度(🔴 阻塞开工 / 🟡 应当澄清 / 🔵 建议)和具体场景的问题清单,末尾给一句总判定。你逐条回应后它会把结论**直接回填进文档**,迭代到一轮无 🔴/🟡 新发现即收敛。

与 `adversarial-review` 的分工:spec-review 轻量、日常、会回填文档,是 clarify 工作流的一部分;adversarial-review 重型、只读、仅显式调用,适合对重要提案做一次性深度红队审查。

### explain-code

按需调用的代码业务逻辑讲解器。指给它一段代码,它不会逐行复述语法,而是讲清楚这段代码在**业务上**做了什么、为什么这么做,并产出一个**自包含的 HTML 解释页**(Tailwind 样式、浅/深色、离线单文件),用具体走查例子 + 可视化图表(流程图、时序图、状态图、时间轴、决策表)来帮你建立心智模型。

仅在显式调用时运行(`/explain-code` 或明确要求用该 skill),不会自动触发。

### story-test

为 Vue 3 + Vite 项目配置 Storybook + Vitest addon,并为组件编写带 `play` 断言的 **stories**,在真实浏览器里逐个状态地验证 UI 行为(每个状态在侧边栏直接显示通过/失败)。适用于:测试 Vue 组件、验证输入框/按钮/表单是否正常、覆盖边界情况(空值、超长文本、纯空白、特殊字符、禁用态)、接入 Storybook、编写 stories、搭建组件/交互测试——即使你没提到 "Storybook" 也会触发。

### adversarial-review

用对抗性思维审查规格类文档(OpenSpec change proposal、设计文档、PRD、RFC)。读完文档后列出关键决策/假设/现状声称的清单,逐条用**假设挖掘、失败场景构造、反方案/魔鬼代言人、歧义遗漏检测**四种手法找茬,有代码库时会用 Grep/Read 交叉核实文档里关于"现状"的声称,再回头自我验证过滤掉纯粹为挑刺而挑刺的伪问题,最后把站得住脚的发现按严重度排序输出。只读,不会帮你改文档。

仅在显式调用时运行(明确要求做对抗性/红队审查),不会在你写文档时自动触发。

### speak-human-zh

中文文风约束。凡是用中文回答问题、解释概念、解读文档、写说明或总结、改写中文文稿时自动生效,即使用户没有明确提出文风要求;用户抱怨回答"太抽象""太口语""看不懂",或要求"别打比方""写正式点"时必须触发。规则:写实际发生的事不打比喻,不用赋能/抓手/闭环这类行业黑话,不用口语词和语气词,不写公文腔和文言,句子主谓清楚、每句话都有具体内容,篇幅不无故变长,输出前逐句自查。用户明确要求打比方、口语风格或面向外行讲解时可放宽。

## License

[MIT](./LICENSE)
